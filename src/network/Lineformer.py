...
class Lineformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Store original input coordinates for skip connections
        # We need the original 3D coordinates, not the encoded features
        original_input = x.clone()  # Store the original input before encoding
        input_pts = original_input  # Use original 3D coordinates for skip connections
