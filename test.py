...
def config_parser():
    ...
    parser.add_argument("--weights", default=f"models/chest.tar", help="path to the experiments")
...
encoder, encoder_output_dim = get_encoder(**cfg["encoder"])
...
# Filter out encoder-related keys that don't exist in the current model
model_state_dict = ckpt["network"]
current_model_state_dict = model.state_dict()

# Only load keys that exist in the current model
filtered_state_dict = {}
for key, value in model_state_dict.items():
    if key in current_model_state_dict:
        filtered_state_dict[key] = value
    else:
        print(f"Skipping key: {key} (not in current model)")

print(f"Loaded {len(filtered_state_dict)} out of {len(model_state_dict)} keys (skipped encoder.embeddings)")
model.load_state_dict(filtered_state_dict, strict=False)

if n_fine > 0:
    # stx()
    model_fine_state_dict = ckpt["network_fine"]
    current_model_fine_state_dict = model_fine.state_dict()
    
    # Only load keys that exist in the current model_fine
    filtered_fine_state_dict = {}
    for key, value in model_fine_state_dict.items():
        if key in current_model_fine_state_dict:
            filtered_fine_state_dict[key] = value
        else:
            print(f"Skipping fine model key: {key} (not in current model)")
    
    print(f"Loaded {len(filtered_fine_state_dict)} out of {len(model_fine_state_dict)} fine model keys")
    model_fine.load_state_dict(filtered_fine_state_dict, strict=False)
